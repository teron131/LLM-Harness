    ...
    if request.tool_call["name"] == "scrape_youtube_tool" and result.status != "error":
        ...
        if isinstance(transcript, str) and transcript.strip():
            ...
            if garbage.garbage_ranges:
                filtered_transcript = filter_content(
                    tagged_transcript, garbage.garbage_ranges
                )
                cleaned_transcript = untag_content(filtered_transcript)
                print(
                    f"🧹 Middleware removed {len(garbage.garbage_ranges)} garbage sections from tool result."
                )
...
def create_summarizer_agent(
    ...
    agent = create_agent(
        ...
        response_format=ToolStrategy(
            Summary
        ),  # Use ToolStrategy for better error handling
    )
...
def summarize_video(
    ...
    prompt = (
        f"Summarize this YouTube video: {transcript_or_url}"
