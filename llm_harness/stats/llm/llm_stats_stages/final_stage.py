...
def _provider_from_model(model: dict[str, Any]) -> str | None:
    """Resolve the provider for Final-stage LLM stats selection."""
    return _provider_from_id(model.get("id")) or (
        model.get("provider_id") if isinstance(model.get("provider_id"), str) else None
    )
...
def _build_speed(
    ...
    throughput = as_finite_number(
        openrouter_speed.get("throughput_tokens_per_second_median")
    ) or as_finite_number(model.get("median_output_tokens_per_second"))
    latency = as_finite_number(
        openrouter_speed.get("latency_seconds_median")
    ) or as_finite_number(model.get("median_time_to_first_token_seconds"))
    e2e_latency = (
        as_finite_number(openrouter_speed.get("e2e_latency_seconds_median"))
        or as_finite_number(model.get("median_time_to_first_answer_token"))
        or latency
    )
    return {
...
def _build_cost(
    model: dict[str, Any],
    openrouter_pricing: dict[str, Any],
...
def _build_intelligence(model: dict[str, Any]) -> dict[str, Any] | None:
    ...
    nonhallucination_rate = as_finite_number(
        intelligence.get("omniscience_hallucination_rate")
    )
...
def _build_intelligence_index_cost(model: dict[str, Any]) -> dict[str, Any] | None:
    ...
    total_cost = as_finite_number(from_row.get("total_cost")) or as_finite_number(
        from_intelligence.get(INTELLIGENCE_COST_TOTAL_COST_KEY)
    )
    total_tokens = as_finite_number(from_row.get("total_tokens")) or as_finite_number(
        from_intelligence.get(INTELLIGENCE_COST_TOTAL_TOKENS_KEY)
    )
    normalized = {
        **from_row,
...
def _project_final_model(
    ...
    pricing = (
        openrouter_pricing_by_id.get(model_id, EMPTY_OPENROUTER_PRICING)
        if model_id
        else EMPTY_OPENROUTER_PRICING
    )
    ...
    return {
        ...
        if isinstance(model.get("attachment"), bool)
        else None,
        ...
        "scores": build_scores(
            model, cost, speed, speed_output_token_anchors, scoring_config
        ),
...
def build_final_payload(
    ...
    *,
    ...
    speed_output_token_anchors = speed_output_token_anchors or [
        200,
        500,
        1000,
        2000,
    ...
    def intelligence_sort_score(model: dict[str, Any]) -> float:
        ...
        score = as_finite_number(
            as_record(model.get("relative_scores")).get("intelligence_score")
        )
